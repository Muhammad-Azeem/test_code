...
class BookingRepository extends BaseRepository
{
    ...
    /**
     * Get user's jobs based on their user_id.
     *
     * @param int $user_id
     * @return array
     */
    ...
    /**
     * Get user's job history based on their user_id.
     *
     * @param int $user_id
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $usertype = 'customer';
            return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => [], 'jobs' => $jobs, 'cuser' => $cuser, 'usertype' => $usertype, 'numpages' => $jobs->lastPage(), 'pagenum' => $jobs->currentPage()];
        } elseif ($cuser && $cuser->is('translator')) {
            $jobs_ids = Job::getTranslatorJobsHistoric($cuser->id, 'historic', $request->get('page', 1));
...
//            $jobs['total'] = $totaljobs;
            return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $noramlJobs, 'jobs' => $jobs, 'cuser' => $cuser, 'usertype' => $usertype, 'numpages' => $numpages, 'pagenum' => $jobs->currentPage()];
        }
    }

    /**
     * Store a new job for the given user.
     *
     * @param  $user
     * @param  $data
     * @return array
...
// Validate input data
        ...
        $immediateTime = 5;
        $isImmediate = $data['immediate'] === 'yes';
...
// Simplify handling of gender and certified data
        $certifiedMapping = [
            'normal' => 'normal',
            'certified' => 'yes',
        ...
        $data['certified'] = $certifiedMapping[array_intersect($certifiedMapping, $data['job_for'])[0]] ?? null;
...
// Determine job_type based on consumer_type
        ...
        $data['will_expire_at'] = $dueCarbon->format('Y-m-d H:i:s');
...
// Fire events or send notifications if required
    ...
    public function storeJobEmail($data)
    {
        ...
        $user = $job->user; // Access the user relationship directly
...
// Save job's information to data for sending Push
        ...
        if ($job->gender && in_array($job->gender, ['male', 'female'])) {
            $data['job_for'][] = ($job->gender == 'male') ? 'Man' : 'Kvinna';
        }

        if ($job->certified && in_array($job->certified, ['yes', 'both', 'n_health', 'law', 'n_law'])) {
            if ($job->certified == 'both') {
